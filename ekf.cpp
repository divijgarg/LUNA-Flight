...
void EKF::priori(float dt, Eigen::Matrix<float, 6, 1> &orientation,FSMState fsm)
{
    Eigen::Matrix<float, 9, 1> xdot = Eigen::Matrix<float, 9, 1>::Zero();

    // angular states from sensors
    Velocity omega_rps; // rads per sec

    omega_rps.vx = orientation(3,0);
    omega_rps.vx = orientation(4,0);
    omega_rps.vx = orientation(5,0);

    euler_t angles_rad;
    angles_rad.yaw = orientation(0,0);
    angles_rad.pitch = orientation(1,0);
    angles_rad.roll= orientation(2,0);
...
void EKF::update(float altitude, Eigen::Matrix<float,3,1> &acceleration, Eigen::Matrix<float,6,1> &orientation, FSMState FSM_state){
    ...
    // ignore altitude measurements after apogee
    ...
    (sensor_accel_global_g)(0, 0) = -acceleration(0,0) + 0.045;
    (sensor_accel_global_g)(1, 0) = acceleration(1,0)- 0.065;
    (sensor_accel_global_g)(2, 0) = acceleration(2,0) - 0.06;
 ...
 */
void EKF::tick(float dt, float sd, float altitude, Eigen::Matrix<float,3,1> &acceleration, Eigen::Matrix<float,6,1> &orientation, FSMState FSM_state){
    ...
    if (FSM_state >= FSMState::STATE_IDLE) //
    {
        ...
        update(altitude, acceleration, orientation, FSM_state);
    }
}
