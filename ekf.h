...
public:
    EKF();
    void initialize();
    void priori(float dt, Eigen::Matrix<float, 6, 1> &orientation, FSMState fsm); 
    void update(float altitude, Eigen::Matrix<float,3,1> &acceleration, Eigen::Matrix<float,6,1> &orientation, FSMState state);
    ...
    void tick(float dt, float sd, float altitude, Eigen::Matrix<float,3,1> &acceleration, Eigen::Matrix<float,6,1> &orientation, FSMState state);
